  ...
  async  function ChangeForm(e) {
    ...
    async function HandleChangeImage(e) {
        ...
        for (let index = 0; index < images.length; index++) {
            ...
            try {
               await axios.post(`${BASEURL}/product-img/add`, formData, {headers: {Authorization: 'Bearer ' + cookie.get('Bearer')}, onDownloadProgress: (progressEvent)=>{console.log(progressEvent);
                }});                
            } catch (err) {
                console.log(err);
            }
      ...
      <div>
          ...
          <form onSubmit={(e)=>addProduct(e)}>
              ...
              <select value={form.category} onChange={(e) => ChangeForm(e)} name="category" className='form-select text-white bg-dark border-dark mb-3' id="category">
                 ...
                 {category.length >0 && category.map((element , index)=><option key={index} value={element.id}>{element.title}</option>)}
              </select>
              ...
              <div onClick={()=>{clickUpload.current.click()}} style={{height:"200px", border:"2px dashed",cursor:openInput?"pointer":"not-allowed"}} className='d-flex align-items-center my-3 justify-content-center'>
                  <h2 className={`${openInput&&"text-effect"}`}>Upload Image</h2>
              </div>
                {images.length > 0 && images.map((element, index) => {
                    return (
                      <div className='position-relative border p-3 rounded-3'>
                        ...
                        <div className='progress'>
                          <div percent={"100%"} style={{width:"100%"}} className='inner-progress progress progress-bar progress-bar-striped progress-bar-animated'></div>
                        </div>
                      </div>
                    );
                })} 
